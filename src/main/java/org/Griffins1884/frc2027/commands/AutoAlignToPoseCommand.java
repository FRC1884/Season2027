...
public class AutoAlignToPoseCommand extends Command {
  ...
  public AutoAlignToPoseCommand(
      ...
      boolean stopOnEnd) {
    this.drive = drive;
    this.target = target == null ? null : (targetInBlueFrame ? AllianceFlipUtil.apply(target) : target);
  ...
  public void initialize() {
    ...
    ChassisSpeeds fieldSpeeds = ChassisSpeeds.fromRobotRelativeSpeeds(robotSpeeds, currentPose.getRotation());
    ...
    double velocityTowardTarget = fieldSpeeds.vxMetersPerSecond * ux + fieldSpeeds.vyMetersPerSecond * uy;
    ...
    double toleranceMeters = Double.isNaN(toleranceOverride)
  ...
  public void execute() {
    ...
    double driveFFVelocity = currentDistance > AlignConstants.Auto.FEEDFORWARD_DEADBAND_METERS.get()
    ...
    double driveVelocityScalar = driveFFVelocity * ffScaler
    ...
    // Command speeds
    var driveVelocity = new Translation2d(driveVelocityScalar, 0.0)
  ...
  private void updateTuningIfChanged(boolean force) {
    ...
    boolean changed = AlignConstants.Auto.TRANSLATION_GAINS.kP().hasChanged(tuningId)
