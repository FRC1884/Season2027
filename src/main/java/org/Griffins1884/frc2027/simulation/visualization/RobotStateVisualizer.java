...
/**
 * Publishes robot components, predicted arcs, and active projectiles for
 * AdvantageScope.
 */
public final class RobotStateVisualizer {
    ...
    private final ProjectileManager projectileManager = new ProjectileManager(shotSimulationConfig.physics());
    ...
    public void periodic() {
        ...
        Rotation2d turretYaw = turret != null ? Rotation2d.fromRadians(turret.getPositionRad()) : new Rotation2d();
        double shooterPivotRotations = superstructure != null && superstructure.getArms().shooterPivot != null
                ? superstructure.getArms().shooterPivot.getPosition()
                : 0.0;

        boolean simTerrainEnabled = GlobalConstants.MODE == RobotMode.SIM;
        Pose3d robotPose3d = simTerrainEnabled
                ? Rebuilt2026FieldModel.terrainAdjustedRobotPose(robotPose)
                : new Pose3d(robotPose);
        Pose3d shooterExitPose3d = ShooterComponentPublisher.createExitPose(
                robotPose3d, turretYaw, shooterPivotRotations, shotSimulationConfig);
        Pose3d turretPose3d = TurretComponentPublisher.createPose3d(robotPose3d, turretYaw, shotSimulationConfig);
        Pose3d shooterPivotPose3d = ShooterComponentPublisher.createPivotPose(
                robotPose3d, turretYaw, shooterPivotRotations, shotSimulationConfig);
        ...
        SuperstructureOutcome outcome = superstructure != null ? superstructure.getLatestOutcome() : null;
        if (superstructure != null && superstructure.isInAllianceZone() && outcome != null) {
            Translation2d turretTarget = outcome.turretTarget();
            Rotation2d commandedTurretYaw = turretTarget != null
                    ? Rotation2d.fromRadians(TurretUtil.turretAngleToTarget(robotPose, turretTarget))
                    : turretYaw;
            double predictedPivotRotations = outcome.shooterPivotManual() ? outcome.shooterPivotPosition()
                    : shooterPivotRotations;
            predictedShot = shotSimulator
                    .solveHubShot(
                            robotPose,
        ...
        if (superstructure != null) {
            boolean shooterReady = superstructure.getRollers().shooter == null
                    || superstructure.getRollers().shooter.isAtGoal();
            boolean armed = predictedShot != null
                    && predictedShot.feasible()
    ...
    private static Translation3d getHubTarget(Pose2d pose) {
        DriverStation.Alliance alliance = DriverStation.getAlliance().orElseGet(() -> inferAllianceFromPose(pose));
