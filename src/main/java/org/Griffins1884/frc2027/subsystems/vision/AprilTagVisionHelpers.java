...
/**
 * This class provides utility methods and record classes for vision-related
 * operations,
 * specifically for pose estimation using April Tags.
 */
public class AprilTagVisionHelpers {
    /**
     * Cool idea by the vision template & the PoseEstimator docs – dynamically
     * update the vision
     * standard deviations based on the distance. In this case, we're scaling the
     * deviations
     * quadratically with distance, so faraway tags are trusted <i>way</i> less than
     * close ones.
     */
    public static Matrix<N3, N1> generateDynamicStdDevs(
            PoseObservation observation, CameraType type) {
        double poseAmbiguityFactor = observation.tagCount() > 1
                ? 1
                : Math.max(
                        1,
